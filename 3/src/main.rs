...
fn walk_grid(end: u32) -> (i32, u32) {
    ...
    for _i in 1..end {
        if found {
            ...
            result = SWEEP
                .iter()
                .map(|sweep| {
                    grid[y.wrapping_add(sweep[0] as usize)][x.wrapping_add(sweep[1] as usize)]
                })
                .sum();
            found = result > end;
            grid[y][x] += &result;
    ...
    (
        (x as i32 - ORIGIN as i32).abs() + (y as i32 - ORIGIN as i32).abs(),
        result,
    )
}
...
fn main() {
    ...
    println!(
        "Manhattan distance: {}, first cluster bigger than input: {}",
        results.0,
        results.1
    )
}
